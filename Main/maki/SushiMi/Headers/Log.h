...
FATAL(msg) : For Fatal level of warning, game breaking error but you dont want it to break the runtime

ERROR(msg): For Error Level of warning, may or may not break game, but still a close to fatal level

WARN(msg): For Lower Level of warning, not game breaking, but causes problems if not properly assigned

INFO(msg): For information printing, printing out information for self use

DEBUG(msg): For debugging purposes,  like checking if values are correct etc

TRACE(msg): For debugging purposes, if unsure where things are breaking, use this all around to check

Example of using:
char* array[10] {0}
If(array[i])
		INFO("The element in the array at position I is %d", array[i]);
else
	   WARN("The element in the array at position I is a '0', Put in a real value!);
