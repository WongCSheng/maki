...
namespace Core
{
	...
	void Levels::Level10()
	{
		...
		if (!Window::loaded)
		{
			...
			fin.open("../Data/Dialogue/lvl10_dialogue.txt");
			if (!fin)
			{
		...
		{
			GameSave::LevelStatusContainer[11].isLevelClear = 1;
			GameSave::LevelStatusContainer[12].isUnlocked = 1; //set next level unlocked
			GameSave::LevelStatusContainer[11].isQuestClear = 1;
			...
			if (SceneManager::spacepressed >= 1)
			{
				...
				{
					Window::isLevel10 = false; //unload curr level
					Window::isLevel11 = true; //load new level
