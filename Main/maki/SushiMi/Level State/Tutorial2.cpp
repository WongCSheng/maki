...
namespace Core
{
	...
	void Levels::Tutorial2()
	{
		if (!Window::loaded)
		{
			...
			Window::dialogue_style = static_cast<int>(Window::dialogue::T2);
			Window::curr_len = 0;
		...
		{
			GameSave::LevelStatusContainer[1].isLevelClear = 1;
			GameSave::LevelStatusContainer[2].isUnlocked = 1; //set next level unlocked
			...
			if (SceneManager::spacepressed >= 1)
			{
				...
				{
					Window::isTut2 = false; //unload curr level
					Window::isLevel1 = true; //load new level
