...
inline char* WriteExponent(int K, char* buffer) {
    ...
    if (K >= 100) {
        *buffer++ = static_cast<char>('0' + static_cast<char>(K / 100));
        K %= 100;
        const char* d = GetDigitsLut() + (uint64_t)K * 2;
        *buffer++ = d[0];
        *buffer++ = d[1];
    }
    else if (K >= 10) {
...
inline char* Prettify(char* buffer, int length, int k, int maxDecimalPlaces) {
    ...
    if (0 <= k && kk <= 21) {
        ...
        // 1234e-2 -> 12.34
        std::memmove(&buffer[kk + 1], &buffer[kk], static_cast<size_t>((uint64_t)length - kk));
        buffer[kk] = '.';
        ...
        // 1234e30 -> 1.234e33
        std::memmove(&buffer[2], &buffer[1], static_cast<size_t>((uint64_t)length - 1));
        buffer[1] = '.';
        buffer[length + 1] = 'e';
