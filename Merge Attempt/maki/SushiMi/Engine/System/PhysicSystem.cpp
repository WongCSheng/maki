...
// F = ma ; accel = F/m
	...
	void PhysicSystem::updatePosition(Physics* ObjectPhysics)
	{
		...
		float acceleration = applyAccel(ObjectPhysics);	 //	stores acceleration
		...
		glm::vec2 prePos = ObjectPhysics.position;				 //	stores previous position/p0

		/*update velocity based on previous velocity*/
		ObjectPhysics.velocity.x = (acceleration * GLHelper::delta_time) + preVel.x; //step1 .x
		ObjectPhysics.velocity.y = (acceleration * GLHelper::delta_time) + preVel.y; //step1 .y


		/*update position based on velocity & previous position*/
		ObjectPhysics.position.x = (1 / 2 * acceleration * GLHelper::delta_time * GLHelper::delta_time) +
			(ObjectPhysics.velocity.x * GLHelper::delta_time) + prePos.x;						//step2	.x	


		ObjectPhysics.position.y = (1 / 2 * acceleration * GLHelper::delta_time * GLHelper::delta_time) +
